...
function getRussianDayOfWeek(dayOfWeek) {
...
$(document).ready(function () {
  ...
  // Расширение jQuery для поддержки $.put() и $.delete()
...
function refreshTimeTable() {
  $.getJSON("/timeTable", function (timeTable) {
    ...
    $("#score").text("Оценка: " + (timeTable.score == null ? "?" : timeTable.score));
    ...
    headerRowByRoom.append($("<th>Время</th>"));
    ...
    headerRowByTeacher.append($("<th>Время</th>"));
    ...
    headerRowByStudentGroup.append($("<th>Время</th>"));
    ...
    $.each(timeTable.timeslotList, (index, timeslot) => {
      ...
      rowByRoom
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                    ${getRussianDayOfWeek(timeslot.dayOfWeek)}
...
function convertToId(str) {
  // Функция для корректного кодирования Unicode-строк в Base64
...
function solve() {
  $.post("/timeTable/solve", function () {
    ...
    showError("Запуск составления расписания не удалось.", xhr);
  });
}
...
function stopSolving() {
  $.post("/timeTable/stopSolving", function () {
    ...
    showError("Прерывание составления расписания не удалось.", xhr);
  });
}
...
function addLesson() {
  ...
  $.post("/lessons", JSON.stringify({
    ...
    showError("Добавление занятия (" + subject + ") не удалось.", xhr);
  });
...
function deleteLesson(lesson) {
  $.delete("/lessons/" + lesson.id, function () {
    ...
    showError("Удаление занятия (" + lesson.name + ") не удалось.", xhr);
  });
}
...
function addTimeslot() {
  $.post("/timeslots", JSON.stringify({
    ...
    showError("Добавление времени занятия не удалось.", xhr);
  });
...
function deleteTimeslot(timeslot) {
  $.delete("/timeslots/" + timeslot.id, function () {
    ...
    showError("Удаление времени занятия (" + timeslot.name + ") не удалось.", xhr);
  });
}
...
function addRoom() {
  ...
  $.post("/rooms", JSON.stringify({
    ...
    showError("Добавление аудитории (" + name + ") не удалось.", xhr);
  });
...
function deleteRoom(room) {
  $.delete("/rooms/" + room.id, function () {
    ...
    showError("Удаление аудитории (" + room.name + ") не удалось.", xhr);
  });
}
...
function showError(title, xhr) {
  ...
  const notification = $(`<div class="toast" role="alert" aria-live="assertive" aria-atomic="true" style="min-width: 30rem"/>`)
    .append($(`<div class="toast-header bg-danger">
                 <strong class="mr-auto text-dark">Ошибка</strong>
...
function pickColor(object) {
  let color = colorMap.get(object);
  if (color !== undefined) {
    return color;
  }
  color = nextColor();
  colorMap.set(object, color);
  return color;
}
...
function buildPercentageColor(floorColor, ceilColor, shadePercentage) {
  let red = ((floorColor & 0xFF0000) +
            Math.floor(shadePercentage * ((ceilColor & 0xFF0000) - (floorColor & 0xFF0000)))) & 0xFF0000;
  let green = ((floorColor & 0x00FF00) +
            Math.floor(shadePercentage * ((ceilColor & 0x00FF00) - (floorColor & 0x00FF00)))) & 0x00FF00;
  let blue = ((floorColor & 0x0000FF) +
            Math.floor(shadePercentage * ((ceilColor & 0x0000FF) - (floorColor & 0x0000FF)))) & 0x0000FF;
